...
for i in range(S.shape[0]):
    if abs(S[i,i] - 1.0) > 1e-14:
        ...
        J3c[:, i, :] *= inverse_sqrt_S_diagonal
        J3c[:, :, i] *= inverse_sqrt_S_diagonal

auxS = auxmol.intor("int1e_ovlp")
for i in range(auxS.shape[0]):
    if abs(auxS[i,i] - 1.0) > 1e-14:
        inverse_sqrt_S_diagonal = 1.0 / np.sqrt(auxS[i,i])
        J3c[i, :, :] *= inverse_sqrt_S_diagonal

np.set_printoptions(linewidth = np.iinfo(np.int32).max, threshold = np.iinfo(np.int32).max, precision = 16, suppress = True)
np.savez_compressed('reference_three_center_all_angular_cartesian_h2_omega_data.npz', J3c = J3c)
