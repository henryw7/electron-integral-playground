...
pyscf_angstrom_integral_threshold = 5e-7

def test_three_center_all_angular_spherical_h2():
...
def test_three_center_all_angular_cartesian_h2():
    molecule = read_xyz_content(
        """2
...
def test_three_center_hof():
    ...
    attach_basis_to_molecule(molecule, basis_set)
    pair_data = form_pair_data(molecule, 1e-14)
    auxiliary_basis_set = read_basis("def2-svp-rifit")
    attach_auxiliary_basis_to_molecule(molecule, auxiliary_basis_set)
    auxiliary_pair_data = form_auxiliary_pair_data(molecule)
    test_J3c = three_center(pair_data, auxiliary_pair_data, molecule)

    ref_J3c = np.load(reference_directory / 'reference_three_center_hof_data.npz')["J3c"]

    np.testing.assert_allclose(test_J3c, ref_J3c, rtol = 0, atol = pyscf_angstrom_integral_threshold)

def test_three_center_hof_distorted():
    molecule = read_xyz_content(
        """3
    ...
    ref_J3c = np.load(reference_directory / 'reference_three_center_hof_distorted_data.npz')["J3c"]

    np.testing.assert_allclose(test_J3c, ref_J3c, rtol = 0, atol = pyscf_angstrom_integral_threshold)

def test_three_center_all_angular_spherical_h2_omega():
    molecule = read_xyz_content(
        """2
    ...
    omega = 0.2
    test_J3c = three_center(pair_data, auxiliary_pair_data, molecule, omega)

    ref_J3c = np.load(reference_directory / 'reference_three_center_all_angular_spherical_h2_omega_data.npz')["J3c"]

    np.testing.assert_allclose(test_J3c, ref_J3c, rtol = 0, atol = pyscf_bohr_integral_threshold)

def test_three_center_all_angular_cartesian_h2_omega():
    molecule = read_xyz_content(
        """2
    ...
    ref_J3c = np.load(reference_directory / 'reference_three_center_all_angular_cartesian_h2_omega_data.npz')["J3c"]

    np.testing.assert_allclose(test_J3c, ref_J3c, rtol = 0, atol = pyscf_bohr_integral_threshold)

def test_three_center_hof_omega():
    molecule = read_xyz_content(
        """3
    ...
    ref_J3c = np.load(reference_directory / 'reference_three_center_hof_omega_data.npz')["J3c"]

    np.testing.assert_allclose(test_J3c, ref_J3c, rtol = 0, atol = pyscf_angstrom_integral_threshold)

def test_three_center_hof_distorted_omega():
    molecule = read_xyz_content(
        """3
    ...
    ref_J3c = np.load(reference_directory / 'reference_three_center_hof_distorted_omega_data.npz')["J3c"]
