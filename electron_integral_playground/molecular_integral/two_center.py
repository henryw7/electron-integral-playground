...
def two_center(pair_data: PrimitivePairDataAngularList, molecule: Molecule) -> np.ndarray:
    ...
    for i_angular, pair_data_i in pair_data.items():
        n_primitive_i = pair_data_i.A_a.shape[0]
        assert n_primitive_i > 0
        for j_angular, pair_data_j in pair_data.items():
            n_primitive_j = pair_data_j.A_a.shape[0]
            assert n_primitive_j > 0
