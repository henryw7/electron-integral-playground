...
def recur_MD_E_term(i: int, j: int, t: int, xyz: str) -> str:
    """
    ...
    if i == 0 and j == 0 and t == 0:
        return "1"
    if j > 0:
        E_t_minus_1 = recur_MD_E_term(i, j-1, t-1, xyz)
        E_t_equal   = recur_MD_E_term(i, j-1, t,   xyz)
        E_t_plus_1  = recur_MD_E_term(i, j-1, t+1, xyz)
        output = ""
        ...
        E_t_minus_1 = recur_MD_E_term(i-1, j, t-1, xyz)
        E_t_equal   = recur_MD_E_term(i-1, j, t,   xyz)
        E_t_plus_1  = recur_MD_E_term(i-1, j, t+1, xyz)
        output = ""
        if E_t_minus_1 is not None: output += f" + one_over_two_p * ({E_t_minus_1})"
...
def get_MD_E_term(i: int, j: int, t: int, xyz: str) -> str:
    assert i >= 0 and j >= 0
    assert t >= 0 and t <= i + j
    assert xyz in [ "x", "y", "z" ]
    term = recur_MD_E_term(i, j, t, xyz)
